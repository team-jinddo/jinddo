...
class TasteAnalyzer:
    ...
    def add_z_score(self, df):  # df: 극성별 개수가 저장된 dataframe
    ...
    def merge_taste_dfs(self, taste_dfs):
    ...
    def run(self):
        taste_dfs = self.split_data_by_taste()
        taste_dfs = {name: self.process_review_df(taste_df, name) for name, taste_df in taste_dfs.items()}
        for name, taste_df in taste_dfs.items():
            taste_df = taste_df.reset_index()
            taste_dfs[name] = taste_df
        merged_df = self.merge_taste_dfs(taste_dfs)
        merged_df.to_csv('./restaurant_taste_scores.csv')

if __name__ == '__main__':
    analyzer = TasteAnalyzer('./review_data_fin_polarity.csv')
    analyzer.run()