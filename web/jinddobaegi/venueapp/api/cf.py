...
class CollaborativeFiltering:
    def __init__(self):
        ...
        self.venueapp_venue = self.check_data("SELECT id, biz_id, score FROM venueapp_venue")
        self.profileapp_profile = self.check_data("SELECT * FROM profileapp_profile")
        self.venuapp_uservisit = self.check_data("SELECT restaurant_id, user_id FROM venueapp_uservisit")
    ...
    def cos_sim_calculator(self, user_taste_vector):
        ...
        scoring_taste_user_based = self.profileapp_profile[
            ['user_id', 'saltiness', 'sourness', 'spiciness', 'sweetness', 'cleanliness']].set_index('user_id')
        cosine_sim = cosine_similarity(scoring_taste_user_based, np.array(user_taste_vector).reshape(1, -1))
        taste_similarity_df = pd.DataFrame(cosine_sim, index=scoring_taste_user_based.index,
                                           columns=['similarity_with_x'])
    ...
    def calculate_pearson_sim_by_taste(self, user_taste_vector):
        ...
        # 결과 DataFrame 생성
        taste_similarity_df = pd.DataFrame(pearson_corr, index=scoring_taste_user_based.index,
                                           columns=['similarity_with_x'])

        return taste_similarity_df.sort_values(by='similarity_with_x', ascending=False).reset_index()

    def calculate_adjusted_cos_sim_by_type(self, user_type_vector):
        type_freq_by_user = self.profileapp_profile[
            ['user_id', 'asian', 'bar', 'bistro', 'buffet', 'chinese', 'dessert', 'donkatsu', 'etc', 'fastfood',
             'japanese', 'korean', 'meat', 'seafood', 'snack', 'soup', 'western']].set_index('user_id')
        ...
        taste_similarity_df = taste_similarity_df.reset_index().rename(
            columns={'index': 'author_id', 0: 'similarity_with_x'}).sort_values(by='similarity_with_x', ascending=False)
    ...
    def calculate_pearson_sim_by_type(self, user_type_vector):
        ...
        taste_similarity_df = taste_similarity_df.reset_index().rename(
            columns={'index': 'user_id', 0: 'similarity_with_x'}).sort_values(by='similarity_with_x', ascending=False)
    ...
    def recommended_bizid(self, visit_list):
        ...
        sorted_score_bizid = self.venueapp_venue[self.venueapp_venue['id'].isin(visit_list)].sort_values(by='score',
                                                                                                         ascending=False)
